...
void MapRenderer::updateMesh()
{
	...
	// when the map is not rendered on an integer scale
	const f32 pad = 0.01f;
	...
	for(u32 j = 0; j < height; ++j)
	{
		for(u32 i = 0; i < width; ++i)
		{
			...
			quad[0].texCoords = sf::Vector2f(tu * tileSize.x + pad, tv * tileSize.y + pad);
			quad[1].texCoords = sf::Vector2f((tu + 1) * tileSize.x - pad, tv * tileSize.y + pad);
			quad[2].texCoords = sf::Vector2f((tu + 1) * tileSize.x - pad, (tv + 1) * tileSize.y - pad);
			quad[3].texCoords = sf::Vector2f(tu * tileSize.x + pad, (tv + 1) * tileSize.y - pad);
		}
	}
